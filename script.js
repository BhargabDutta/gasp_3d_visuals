...
let urls = new Array(frameCount).fill().map((o, i) => `frames/frame${(i + 1).toString().padStart(4, '0')}.png`);

// Calling the imageSequence function with the local image URLs
imageSequence({
  urls, // Array of image paths (local files in the "frames" folder)
...
function imageSequence(config) {
  ...
  // Load images from the local "frames" folder
  images = config.urls.map((url, i) => {
    let img = new Image();
    img.src = url;
    i || (img.onload = updateImage); // Ensure the first image triggers the update
    return img;
  });

  // Return the tween that will drive the scroll-based animation
  return gsap.to(playhead, {
    frame: images.length - 1,
    ease: "none",
    onUpdate: updateImage,
    duration: images.length / (config.fps || 30), // Optional FPS control
